...
TMeasuresInfo *AppGetMeasure ()
{
	...
	gchar *unit1, *unit2;
	int i;
	combo1 = (GtkComboBox *) lookup_widget(GTK_WIDGET(App), "combo_unit1");
	combo2 = (GtkComboBox *) lookup_widget(GTK_WIDGET(App), "combo_unit2");
	unit1 = ComboGetActiveText(combo1);
	unit2 = ComboGetActiveText(combo2);
...
void AppReadWriteConfig (int opmode)
{
	...
	key = g_malloc0(512);
	sval = g_malloc0(512);
	sprintf(configfile, "%s/gmeasures.cfg", getenv("XDG_CONFIG_HOME"));
	if(opmode == OF_WRITE) {
